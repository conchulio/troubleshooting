...
var Measurement = function(isWorker) {
  ...
  if (!isWorker) {
    ...
    $(function() {
      ...
      var schedulePeriodicMeasurement = function() {
      ...
      $('.btn-measurement-interval').click(schedulePeriodicMeasurement);

      // if (/Netalyzr$/.test(navigator.userAgent)) {
      // console.log("Before trying to decide whether it's Android or not", navigator.userAgent);
      if (window.Android) {
        console.log("Detected an Android device!");
        window.addEventListener('finishedinitialization', function (event) {
          console.log("Everything ready event was fired!");
          that.performMeasurement();
        }, false);
      } else {
        // setTimeout(schedulePeriodicMeasurement, 5000);
      }
    });
    ...
    for (var i = 0; i < measurementsToDoRaw.length; i++) {
      if (measurementsToDoRaw[i].method.match(/ajax/) && measurementsToDoRaw[i].times>0) {
        ...
        ajaxPingWorker.onmessage = function(event) {
          ...
          that.initializedWorkers += 1;
          if (Object.keys(that.workers).length === that.initializedWorkers) {
            window.dispatchEvent(new Event("finishedinitialization"));
          }
        };
    ...
    postMessage(["finishedinitialization"]);
  }
}
...
Measurement.prototype.establishPeerConnection = function(peerName, isBoss, message) {
  ...
  function start(isBoss) {
    ...
    pc.onicecandidate = function (event) {
      ...
      that.socket.sendWrapper(JSON.stringify({ "recipientSocketId": peerName, isBoss: isBoss, "candidate": event.candidate }));
    };
    ...
    if (isBoss) {
      pc.createOffer(function(desc) {
        ...
        that.socket.sendWrapper(JSON.stringify({ os: Measurement.normalizeOs(platform.os.family), browser: platform.name, recipientSocketId: peerName, isBoss: isBoss, "sdp": desc }));
      ...
      pc.setRemoteDescription(new RTCSessionDescription(message.sdp), function() {
        pc.createAnswer(function(desc) {
          ...
          that.socket.sendWrapper(JSON.stringify({ os: Measurement.normalizeOs(platform.os.family), browser: platform.name, sdp: desc, isBoss: isBoss, recipientSocketId: message.senderSocketId }));
        }, function (err) {throw err;});
      }, function (err) {throw err;});
    }
  }
...
Measurement.prototype.establishSocketConnection = function() {
  ...
  this.socket.onclose = function(event) {
    ...
    for (var variable in pcPeerHash) {
      if (pcPeerHash.hasOwnProperty(variable)) {
        pcPeerHash[variable].close();
      }
    }
  };

  this.socket.sendWrapper = function(message) {
    if (this.readyState === 2 || this.readyState === 3) {
      that.establishSocketConnection();
    }
    this.send(message);
  };
  ...
  this.socket.onmessage = function(event) {
    ...
    if (message.type==="startWebRTC") {
      ...
      if (window.Android) {
        Android.signalWebviewFinish();
      }
      that.results = message.results;
      for (var i = 0; i < message.results.length; i++) {
        message.results[i] = message.results[i] || {};
      }
...
Measurement.prototype.performMeasurement = function() {
  ...
  measurementsToDo.forEach(function(item) {
    ...
    var whenToSend = 0;
    if (index!==1) {
      setTimeout(function(measurementTask) {that.socket.sendWrapper(JSON.stringify(measurementTask));}, whenToSend, measurementTask);
    } else {
      ...
      allPeers.forEach(function(otherSocketId) {
        setTimeout(function(measurementTask) {
          ...
          that.socket.sendW(JSON.stringify(measurementTask));
...
Measurement.prototype.transmitResultsToDb = function() {
  ...
  var resultsToSend = {type:"finishedResults", recipientSocketId:"database", experimentStartedAt:this.experimentStartedAt, resultsLength: this.results.resultsLength, results:this.results};
  console.log("resultsToSend", resultsToSend);
  this.socket.sendWrapper(JSON.stringify(resultsToSend));
};
...
Measurement.prototype.sendResults = function(thing, destination, batchNumber) {
  if (arguments.length===1) {
    ...
    this.socket.sendWrapper(JSON.stringify(arguments[0]));
    return;
  }
  ...
  this.socket.sendWrapper(JSON.stringify(this.resultsToTransmit[thing+'-'+destination+'-'+batchNumber]));
