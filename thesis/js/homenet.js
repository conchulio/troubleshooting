...
function colorize(d) {
  if (d.betterThan) {
    return "hsl("+Math.round(d.betterThan*120)+", 100%, 50%)";
  } else {
    return "darkgray";
  }
}
...
var NetGraph = function(elem, clickevents, width) {
    ...
    var width = this.width = (width || canvas.width());
    // var width = this.width = (width || 0.85*canvas.width());
    var height = canvas.height();
    // var height = this.height = width/2.5;
    ...
    var nodemouseout = function () {
        ...
        d3.select(this).select("circle").transition()
            .duration(300)
            .attr("r", function(item) {return (3/4)*getdefaultr(item);})
            .attr("stroke-width", function(item) {return (1/4)*getdefaultr(item);});
    };
    ...
    var redraw = this.redraw = function() {
        ...
        node.attr("class", function(n) {
            ...
            case 'peer':
                var brokenNode = n.raw.iceConnectionState==='disconnected' ? ' broken' : '';
                // console.log('iceState', n.raw.iceConnectionState, 'brokenNode', brokenNode);
                return 'node peer-node'+brokenNode;
                break;
        ...
        // console.log('node', node);
        node.select("circle").style("stroke", colorize);
        node.exit().remove();
...
NetGraph.prototype.removeNode = function(node) {
  var that = this;
  var nodeIndex = that.nodes.indexOf(node);
  if (nodeIndex < 0) {
    return;
  }
  // console.log('nodeIndex', nodeIndex);
  // console.log("delete node", node);
  that.nodes.splice(nodeIndex, 1);
  // console.log('nodes', that.nodes);
  var edge = null;
  while (edge = that.isConnected(node)) {
    var edgeIndex = that.links.indexOf(edge);
    if (edgeIndex > -1) {
      // console.log("delete edge", edge);
      that.links.splice(edgeIndex, 1);
      // console.log('edges', that.links);
    }
  }
};
...
NetGraph.prototype.addNode = function(newnode) {
    ...
    var node = _.find(that.nodes, function(n) {
        return (n.type === newnode.type && (n.ipv4!==undefined && n.ipv4 === newnode.ipv4) ||
    ...
    if (!node) {
        ...
        case 'internet':
            ...
            node.x = Number($('#canvas').attr('width'))*0.5;
            node.y = Number($('#canvas').attr('height'))*0.65;
            node.fixed = true;
            that.internet = node;
            break;
        ...
        case 'local':
            ...
            that.localnode = node;
            node.selfBetterThan = newnode.selfBetterThan;
            node.betterThan = newnode.selfBetterThan;
            node.routerBetterThan = newnode.routerBetterThan;
            break;
    ...
    _.each(gws, function(gw) {
        // local - > gw
        if (that.localnode && gw.reachable) {
            // if (!that.hasEdge(that.localnode, gw)) {
                // console.log("Debug remove better than")
                // console.log("Redrawing node!!!", newnode);
                that.addEdge(that.localnode, gw, that.localnode.routerBetterThan);
            // }
        }
        ...
        _.each(_.filter(that.nodes, function (n) { return (n.type === 'peer'); }), function(peer) {
            // if (!that.isConnected(peer)) {
                ...
                if (longestgw && (bitsv6 > 0 || bitsv4 > 0)) {
                    that.addEdge(peer, longestgw, peer.routerBetterThan);
                }
            // }
        });
    }
...
NetGraph.prototype.addEdge = function(a, b, betterThan) {
    // console.log("this.links", JSON.parse(JSON.stringify(this.links)));
    // console.log('adding Edge: a, b', a, b);
    var existingEdge = this.hasEdge(a, b);
    // console.log('existingEdge', existingEdge);
    // console.log(JSON.parse(JSON.stringify(existingEdge)));
    if (existingEdge) {
      // console.log("Existing edge!", existingEdge.source.ipv4, existingEdge.target.ipv4, betterThan);
      existingEdge.betterThan = betterThan || existingEdge.betterThan;
    } else {
      // console.log("Adding new edge!", a.ipv4, b.ipv4, betterThan);
      this.links.push({
          source : a,
          target : b,
